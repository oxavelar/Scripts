...
$LIBRETRO_PATH/libretro-install.sh $(realpath $OUT_DIR)

# Organize our files in a portable structure
mkdir -p "$OUT_DIR/cores-info" "$OUT_DIR/cores" "$OUT_DIR/shaders" "$OUT_DIR/lib" "$OUT_DIR/autoconf/" "$OUT_DIR/downloads/" "$OUT_DIR/system/"  "$OUT_DIR/screenshots/" "$OUT_DIR/assets/" "$OUT_DIR/overlay/" "$OUT_DIR/saves/" "$OUT_DIR/roms/" "$OUT_DIR/remap/" "$OUT_DIR/cheats/"
find "$OUT_DIR" -iname *.info -exec mv -f \{\} "$OUT_DIR/cores-info/" 2> /dev/null \;
find "$OUT_DIR" -iname *.so -exec mv -f \{\} "$OUT_DIR/cores/" 2> /dev/null \;
mv -f "$OUT_DIR/tmp/usr/local/bin" "$OUT_DIR"
...
# Convert shaders
$LIBRETRO_PATH/retroarch/tools/cg2glsl.py "$LIBRETRO_PATH/retroarch/media/shaders_cg" "$LIBRETRO_PATH/retroarch/media/shaders_glsl"
